...
interface ASECloneOptionsModalProps {
    isOpen: boolean;
    onClose: () => void;
    sourceServer: AseServer;
    allServers: AseServer[];
...
export default function ASECloneOptionsModal({
    isOpen,
    onClose,
    sourceServer,
    allServers,
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
                ...
                <div className="flex items-center justify-between p-5 border-b border-slate-700/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-white">Server Actions (ASE)</h2>
                ...
                <div className="p-5 space-y-3">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className={cn(
                                "p-3 rounded-lg",
                                selectedAction === 'clone' ? "bg-amber-500/20" : "bg-slate-700/50"
                            )}>
                                <Copy className={cn(
                                    "w-5 h-5",
                                    selectedAction === 'clone' ? "text-amber-400" : "text-slate-400"
                                )} />
                            </div>
                            <div>
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className={cn(
                                "p-3 rounded-lg",
                                selectedAction === 'transfer' ? "bg-cyan-500/20" : "bg-slate-700/50"
                            )}>
                                <Settings className={cn(
                                    "w-5 h-5",
                                    selectedAction === 'transfer' ? "text-cyan-400" : "text-slate-400"
                                )} />
                            </div>
                            <div>
                                ...
                                <div className="space-y-2 max-h-40 overflow-y-auto">
                                    {targetServers.map(server => (
                                        ...
                                        >
                                            ...
                                            <span className="text-xs text-slate-500 ml-auto">{server.mapName}</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                ...
                <div className="flex items-center justify-end gap-3 p-5 border-t border-slate-700/50 bg-slate-800/30">
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            selectedAction === 'clone' && "bg-amber-500 hover:bg-amber-400 text-black",
                            selectedAction === 'transfer' && "bg-cyan-500 hover:bg-cyan-400 text-slate-950",
