...
export default function ClusterManager() {
    ...
    const [newclusterDir, setNewclusterDir] = useState('');
    ...
    const fetchClusters = async () => {
        setIsLoading(true);
        try {
            const data = await getAseClusters();
            setClusters(data);
            ...
            for (const cluster of data) {
                try {
                    statuses[cluster.id] = await getAseClusterCrossChatStatus(cluster.id);
                } catch {
    ...
    const handleValidateCluster = async (clusterId: number) => {
        ...
        try {
            const result = await validateAseClusterConfiguration(clusterId);
    ...
    const getServerStatus = (serverId: number): Server | undefined => {
        return servers.find(s => s.id === serverId) as unknown as Server | undefined;
    };
    ...
    const handleToggleCrossChat = async (clusterId: number) => {
        ...
        try {
            await toggleAseClusterCrossChat(clusterId, !currentStatus);
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <input
                                    type="text"
                                    value={newclusterDir}
                                    onChange={(e) => setNewclusterDir(e.target.value)}
                                ...
                                />
                                <button
                                    onClick={handleBrowseclusterDir}
                        ...
                        <div key={cluster.id} className="glass-panel rounded-2xl p-6 relative group">
                            ...
                            <div className="mt-4">
                                <VisualClusterBuilder
                                    cluster={cluster}
                                    servers={servers as unknown as Server[]}
                                    allServers={servers as unknown as Server[]}
