...
import {
    getBoostProfiles, saveBoostProfile, deleteBoostProfile,
    getActiveBoostProfile, activateBoostProfile, deactivateBoostProfile,
    getAllServers
} from '../../utils/tauri';
import { type BoostProfile } from '../../types';
...
export default function BoostManager() {
    ...
    const [profiles, setProfiles] = useState<BoostProfile[]>([]);
    const [activeProfile, setActiveProfile] = useState<BoostProfile | null>(null);
    ...
    const [editingProfile, setEditingProfile] = useState<BoostProfile | null>(null);
    ...
    useEffect(() => {
        getAllServers()
            .then((s) => {
                const asaServers = s.filter(srv => srv.serverType === 'ASA');
                if (asaServers.length > 0) setSelectedServerId(asaServers[0].id);
            })
            .catch(console.error);
    }, []);
    ...
    const loadProfiles = async () => {
        ...
        setIsLoading(true);
        try {
            const pList = await getBoostProfiles(selectedServerId);
            setProfiles(pList);
            const active = await getActiveBoostProfile(selectedServerId);
    ...
    const handleOpenEditModal = (profile: BoostProfile) => {
    ...
    const handleSaveProfile = async (e: React.FormEvent) => {
        ...
        try {
            const profileData: BoostProfile = {
            ...
            await saveBoostProfile(profileData);
    ...
    const handleActivateProfile = async (profile: BoostProfile) => {
        ...
        try {
            ...
            await activateBoostProfile(selectedServerId, profile.id);
    ...
    const handleDeactivateBoost = async () => {
        ...
        try {
            ...
            await deactivateBoostProfile(selectedServerId);
                    ...
                    {profiles.map((profile) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="border-t border-slate-880/60 p-4 bg-slate-900/[0.15]">
                                    {isActive ? (
