...
export const ConfigTooltip = ({
    ...
    const tooltipContent = (description || defaultValue) ? (
        <AnimatePresence>
            {isVisible && (
                <motion.div
                    ...
                    className="fixed z-[9999] w-72 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl p-4 pointer-events-none origin-bottom-left"
                >
                    ...
                    <div className="absolute bottom-0 left-4 translate-y-full overflow-hidden h-2 w-4">
                        <div className="w-3 h-3 bg-slate-900 border-r border-b border-white/10 rotate-45 -translate-y-1.5 translate-x-0.5" />
                    </div>
                </motion.div>
            )}
        </AnimatePresence>
