...
use crate::services::ase_discord_bridge::AseDiscordBridgeConfig;
use crate::services::discord::{DiscordEmbed, send_discord_webhook};
...
#[tauri::command]
pub async fn save_ase_discord_bridge_config(
    state: State<'_, AppState>,
    config: AseDiscordBridgeConfig,
) -> Result<(), String> {
    // 1. Save to Database
    {
        ...
        let exists: i32 = conn
            .query_row(
                "SELECT COUNT(*) FROM ase_discord_bridge_config WHERE cluster_id = ?1",
                [config.cluster_id],
                |row| row.get(0),
            )
            .unwrap_or(0);
        ...
        if exists > 0 {
            conn.execute(
                "UPDATE ase_discord_bridge_config SET 
            ...
            conn.execute(
                "INSERT INTO ase_discord_bridge_config (
    ...
    state.ase_discord_bridge.configure(config.clone()).await;
...
#[tauri::command]
pub async fn get_ase_discord_bridge_config(
    state: State<'_, AppState>,
    cluster_id: i64,
) -> Result<Option<AseDiscordBridgeConfig>, String> {
    // 1. Try Memory First
    if let Some(cfg) = state.ase_discord_bridge.get_config().await {
    ...
    // 2. Fallback to DB
    let result: Result<AseDiscordBridgeConfig, rusqlite::Error> = {
        let db = state.db.lock().map_err(|e| e.to_string())?;
        let conn = db.get_connection().map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn start_ase_discord_bridge(state: State<'_, AppState>) -> Result<(), String> {
    if !state.ase_discord_bridge.is_running() {
        state.ase_discord_bridge.clone().start();
        Ok(())
    } else {
        Ok(()) // Already running
...
#[tauri::command]
pub async fn stop_ase_discord_bridge(state: State<'_, AppState>) -> Result<(), String> {
    state.ase_discord_bridge.stop();
    Ok(())
}
...
#[tauri::command]
pub async fn test_ase_discord_bridge_connection(
    state: State<'_, AppState>,
    bot_token: String,
    channel_id: String,
...
#[tauri::command]
pub async fn generate_ase_bot_invite_url(bot_token: String) -> Result<String, String> {
...
#[tauri::command]
pub async fn send_ase_discord_status_update(
    state: State<'_, AppState>,
    ...
    let servers = {
        ...
        let mut stmt = conn
            .prepare("SELECT id, name, status, map_name, max_players FROM ase_servers")
            .map_err(|e| e.to_string())?;
    ...
    let online_servers: Vec<(String, String, i32, i32)> = servers
        ...
        .map(|(id, name, _, map, max): &(i64, String, ServerStatus, String, i32)| {
...
#[tauri::command]
pub async fn get_ase_discord_rate_limit_config(
    state: State<'_, AppState>,
    cluster_id: i64,
    ...
    let result = conn.query_row(
        "SELECT max_messages_per_window, window_seconds FROM ase_discord_rate_limits WHERE cluster_id = ?1",
        [cluster_id],
        |row| {
...
#[tauri::command]
pub async fn set_ase_discord_rate_limit_config(
    state: State<'_, AppState>,
    cluster_id: i64,
...

    {
        ...
        let exists: i32 = conn
            .query_row(
                "SELECT COUNT(*) FROM ase_discord_rate_limits WHERE cluster_id = ?1",
                [cluster_id],
                |row| row.get(0),
            )
            .unwrap_or(0);
        ...
        if exists > 0 {
            conn.execute(
                "UPDATE ase_discord_rate_limits SET max_messages_per_window = ?1, window_seconds = ?2, updated_at = CURRENT_TIMESTAMP WHERE cluster_id = ?3",
                rusqlite::params![max_messages_per_window, window_seconds, cluster_id],
            )
        } else {
            conn.execute(
                "INSERT INTO ase_discord_rate_limits (cluster_id, max_messages_per_window, window_seconds) VALUES (?1, ?2, ?3)",
    ...
    if let Some(cfg) = state.ase_discord_bridge.get_config().await {
        if cfg.cluster_id == cluster_id {
            ...
            if let Some((max_msgs, window_secs)) = state.ase_discord_bridge.load_rate_limit_config(cluster_id) {
                let mut limiter = state.ase_discord_bridge.rate_limiter.lock().await;
