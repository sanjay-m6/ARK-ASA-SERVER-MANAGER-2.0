...
#[tauri::command]
pub async fn get_ase_boost_profiles(
    state: State<'_, AppState>,
    server_id: i64,
) -> Result<Vec<AseBoostProfile>, String> {
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id, server_id, name, xp_multiplier, taming_multiplier, harvest_multiplier, \
             ...
             FROM ase_boost_profiles WHERE server_id = ?1"
        )
...
#[tauri::command]
pub async fn save_ase_boost_profile(
    state: State<'_, AppState>,
    profile: AseBoostProfile,
) -> Result<(), String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    let conn = db.get_connection().map_err(|e| e.to_string())?;
    ...
    if let Some(id) = profile.id {
        conn.execute(
            "UPDATE ase_boost_profiles SET name = ?1, xp_multiplier = ?2, taming_multiplier = ?3, \
        ...
        conn.execute(
            "INSERT INTO ase_boost_profiles (server_id, name, xp_multiplier, taming_multiplier, harvest_multiplier, \
             mating_multiplier, hatch_multiplier, mature_multiplier, active) \
...
#[tauri::command]
pub async fn delete_ase_boost_profile(
    state: State<'_, AppState>,
    id: i64,
) -> Result<(), String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    ...
    conn.execute("DELETE FROM ase_boost_profiles WHERE id = ?1", [id])
        .map_err(|e| e.to_string())?;
    Ok(())
}

#[tauri::command]
pub async fn get_active_ase_boost_profile(
    state: State<'_, AppState>,
    server_id: i64,
) -> Result<Option<AseBoostProfile>, String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    let conn = db.get_connection().map_err(|e| e.to_string())?;
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id, server_id, name, xp_multiplier, taming_multiplier, harvest_multiplier, \
             mating_multiplier, hatch_multiplier, mature_multiplier, active \
             FROM ase_boost_profiles WHERE server_id = ?1 AND active = 1"
        )
        .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn activate_ase_boost_profile(
    ...
    let profile = {
        ...
        conn.query_row(
            "SELECT id, server_id, name, xp_multiplier, taming_multiplier, harvest_multiplier, \
             mating_multiplier, hatch_multiplier, mature_multiplier FROM ase_boost_profiles WHERE id = ?1",
            [id],
            |row| {
                Ok(AseBoostProfile {
                    id: Some(row.get(0)?),
    ...
    let _ = deactivate_ase_boost_profile_internal(&state, server_id).await;
    ...
    // 6. Update database active status
    {
        let db = state.db.lock().map_err(|e| e.to_string())?;
        let conn = db.get_connection().map_err(|e| e.to_string())?;
        conn.execute("UPDATE ase_boost_profiles SET active = 0 WHERE server_id = ?1", [server_id]).map_err(|e| e.to_string())?;
        conn.execute("UPDATE ase_boost_profiles SET active = 1 WHERE id = ?1", [id]).map_err(|e| e.to_string())?;
    }

    // 7. Restart server
    let _ = crate::ase::commands::server::stop_ase_server(server_id, state.clone()).await;
    tokio::time::sleep(std::time::Duration::from_secs(5)).await;
    let _ = crate::ase::commands::server::start_ase_server(app_handle, server_id, state.clone()).await;

    Ok(())
}

#[tauri::command]
pub async fn deactivate_ase_boost_profile(
    app_handle: AppHandle,
    state: State<'_, AppState>,
    server_id: i64,
) -> Result<(), String> {
    deactivate_ase_boost_profile_internal(&state, server_id).await?;
...
pub async fn deactivate_ase_boost_profile_internal(
    state: &State<'_, AppState>,
    server_id: i64,
) -> Result<(), String> {
