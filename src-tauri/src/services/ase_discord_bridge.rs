...
pub struct AseDiscordBridgeConfig {
    pub cluster_id: i64,
    pub enabled: bool,
...
impl Default for AseDiscordBridgeConfig {
    fn default() -> Self {
        Self {
            cluster_id: 0,
            enabled: false,
...
struct GatewayHandler {
    ...
    config: Arc<Mutex<Option<AseDiscordBridgeConfig>>>,
...
impl GatewayHandler {
    ...
    fn roles_match(user_roles: &[String], cfg: &AseDiscordBridgeConfig, level: PermLevel) -> bool {
...
impl SerenityEventHandler for GatewayHandler {
            ...
            let content = match cmd_name {
                ...
                "restart" => {
                    if let Some(id) = server_id {
                        ...
                        tauri::async_runtime::spawn(async move {
                            if let Some(state) = app.try_state::<AppState>() {
                                let _ = crate::ase::commands::server::restart_ase_server(app.clone(), id, None, state).await;
                            }
                        });
            ...
            let mut server_ids: Vec<i64> = Vec::new();
            {
                ...
                if let Ok(db) = db_result {
                    if let Ok(conn) = db.get_connection() {
                        if let Ok(mut stmt) = conn.prepare("SELECT id FROM ase_servers WHERE cluster_id = ?1") {
    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        if !config.admin_channel_id.is_empty() && msg.channel_id.to_string() == config.admin_channel_id {
            ...
            let state = self.app_handle.state::<AppState>();
            let service = state.ase_discord_bridge.clone();
            
            if service.should_relay_from_discord(&msg.author.id.to_string(), &msg.content, msg.author.bot).await {
                let formatted = AseDiscordBridgeService::format_for_game(&msg.author.name, &msg.content);
        ...
        match command {
            ...
            "!start" => {
                ...
                if let Ok(id) = args[0].parse::<i64>() {
                    let current_status: Option<String> = {
                        ...
                        locked.as_ref().and_then(|db| db.get_connection().ok().and_then(|conn| conn.query_row("SELECT status FROM ase_servers WHERE id = ?1", [id], |row| row.get::<_, String>(0)).ok()))
                    };
            ...
            "!restart" => {
                ...
                if let Ok(id) = args[0].parse::<i64>() {
                    ...
                    tauri::async_runtime::spawn(async move {
                        if let Some(state) = app.try_state::<AppState>() {
                            match crate::ase::commands::server::restart_ase_server(app.clone(), id, None, state).await {
...
async fn get_all_servers_status(state: &AppState) -> Result<Vec<ClusterServerInfo>, String> {
    ...
    let mut stmt = conn
        .prepare("SELECT id, name, status FROM ase_servers")
...
#[derive(Clone)]
pub struct AseDiscordBridgeService {
    app_handle: AppHandle,
...
impl AseDiscordBridgeService {
    ...
    pub async fn get_config(&self) -> Option<AseDiscordBridgeConfig> {
        let cfg = self.config.lock().await;
        cfg.clone()
    }
    ...
    pub fn load_config_from_db(&self) -> Option<AseDiscordBridgeConfig> {
        use tauri::Manager;
    ...
    async fn update_server_list(&self, config: &AseDiscordBridgeConfig) -> Result<(), String> {
    ...
    async fn update_player_list(&self, config: &AseDiscordBridgeConfig) -> Result<(), String> {
    ...
    fn save_message_id_to_db(&self, cluster_id: i64, col_name: &str, new_id: &str) {
        ...
        if let Ok(db) = state.db.lock() {
            if let Ok(conn) = db.get_connection() {
                let sql = format!(
                    "UPDATE ase_discord_bridge_config SET {} = ?1 WHERE cluster_id = ?2",
                    col_name
                );
        ...
        if let Some(state) = self.app_handle.try_state::<AppState>() {
            if let Ok(db_guard) = state.db.lock() {
                if let Ok(conn) = db_guard.get_connection() {
                    let _ = conn.execute(
                        "INSERT INTO ase_discord_sent_messages (cluster_id, message_hash, excerpt) VALUES (?1, ?2, ?3)",
                        params![cluster_id, message_hash, excerpt],
                    );
                    // Optionally prune old messages (keep recent 1000)
                    let _ = conn.execute(
                        "DELETE FROM ase_discord_sent_messages WHERE id NOT IN (SELECT id FROM ase_discord_sent_messages ORDER BY created_at DESC LIMIT 1000)",
                        [],
                    );
                }
            }
    ...
    fn recent_message_exists(&self, cluster_id: i64, message_hash: &str, window_seconds: i64) -> Result<bool, String> {
        ...
        let count: i64 = {
            ...
            let sql = "SELECT COUNT(1) FROM ase_discord_sent_messages WHERE cluster_id = ?1 AND message_hash = ?2 AND created_at > datetime('now', ?3)";
    ...
    fn fetch_cluster_servers(&self, cluster_id: i64) -> Result<Vec<ClusterServerInfo>, String> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, name, status, max_players, last_started FROM ase_servers WHERE cluster_id = ?1"
        ).map_err(|e| e.to_string())?;
    ...
    fn fetch_server_names(&self, cluster_id: i64) -> Result<HashMap<i64, String>, String> {
        ...
        let conn = db.get_connection().map_err(|e| e.to_string())?;
        let mut stmt = conn
            .prepare("SELECT id, name FROM ase_servers WHERE cluster_id = ?1")
            .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn test_message_format() {
        let formatted = AseDiscordBridgeService::format_for_game("TestUser", "Hello world");
